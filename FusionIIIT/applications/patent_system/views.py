...
def forward_application(request, application_id):
    if request.method == "POST":
        try:
            ...
            _notify(
                application,
                f"Application {application.title} has been forwarded to the Director.",
...
def attorney_forward_to_director(request, app_id):
    ...
    allowed_statuses = {"Attorney Assigned", "Needs Revision", "Returned to Director"}
    if application.status not in allowed_statuses:
        return JsonResponse(
            {
                "error": (
                    "Application must be in one of the statuses "
                    f"{sorted(allowed_statuses)}. Current status: {application.status}"
                )
            },
            status=400,
        )
...
def attorney_applications(request):
    ...
    applications = (
        Application.objects.filter(
            attorney=attorney,
            status__in=["Attorney Assigned", "Returned to Director", "Needs Revision"],
        )
    ...
    for application in applications:
        ui_status = (
            "Needs Revision"
            if application.status == "Needs Revision" or application.decision_status == "Needs Revision"
...
def _get_attorney_for_user(user):
...
def _is_attorney_user(user):
    role_names = _role_names(user)
    if any("attorney" in role for role in role_names):
        return True
    return _get_attorney_for_user(user) is not None


def _require_comments(payload, key="comments"):
...
def _create_audit(action, actor, application=None, details=""):
    AuditLog.objects.create(action=action, actor=actor, application=application, details=details)


def _notify(application, message, recipient=None, recipient_role=None, event_type="General", due_date=None):
...
def _reviewer_workload(user):
    return Application.objects.filter(assigned_pcc_admin=user, status__in=["Submitted", "Reviewed by PCC Admin", "Needs Revision"]).count()
...
def withdraw_application(request, app_id):
    ...
    _create_audit('Withdraw Application', request.user, application, 'Applicant withdrew application')
...
def resubmit_application(request, app_id):
    ...
    _create_audit('Resubmit Application', request.user, application, 'Applicant resubmitted revised application')
...
def declare_conflict(request, app_id):
    ...
    if serializer.is_valid():
        ...
        _create_audit('Conflict Declared', request.user, application, serializer.validated_data.get('conflict_type', ''))
        return Response(serializer.data, status=status.HTTP_201_CREATED)
    return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
def director_decide_budget(request, app_id):
    ...
    _create_audit('Budget Decision', request.user, application, budget.status)
...
def respond_office_action(request, office_action_id):
    ...
    if serializer.is_valid():
        ...
        _create_audit('Office Action Responded', request.user, office_action.application, office_action.action_reference)
        return Response(serializer.data, status=status.HTTP_201_CREATED)
    return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
