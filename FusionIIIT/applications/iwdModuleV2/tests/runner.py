...
def generate_execution_log(records, tester_name):
    ...
    for rec in records:
        ...
        source_type = 'UC' if rec.get('uc_id') else 'BR' if rec.get('br_id') else 'WF' if rec.get('wf_id') else ''
        source_id = rec.get('uc_id') or rec.get('br_id') or rec.get('wf_id') or ''
        outcome = rec.get('outcome', 'N/A')
        results = rec.get('results', [])
        actual = results[-1].get('actual', '') if results else ''
        evidence = results[-1].get('evidence', '') if results else ''
        status_from_results = results[-1].get('status', outcome) if results else outcome
        rows.append([test_id, source_type, source_id, rec.get('expected_result', ''), actual, status_from_results, evidence, tester_name])
    _write_csv('Test_Execution_Log.csv', ['Test ID', 'Source Type', 'Source ID', 'Expected Result', 'Actual Result', 'Status', 'Evidence', 'Tester'], rows)
    return rows
...
def generate_defect_log(records):
    ...
    for rec in records:
        if rec.get('outcome') not in ('Fail', 'Error'):
            continue
        related_artifact = rec.get('uc_id') or rec.get('br_id') or rec.get('wf_id') or ''
        severity = 'Critical' if rec.get('outcome') == 'Error' else 'High'
        description = str(rec.get('error', '') or rec.get('scenario', '') or 'Test failed')[:500]
        suggested_fix = 'Review the failing endpoint, permissions, or expected preconditions; then rerun the related test.'
        rows.append([f'DEF-{defect_no:03d}', rec.get('test_id', 'N/A'), related_artifact, severity, description, suggested_fix])
        defect_no += 1
    _write_csv('Defect_Log.csv', ['Defect ID', 'Related Test ID', 'Related Artifact', 'Severity', 'Description', 'Suggested Fix'], rows)
    return rows
...
def generate_artifact_evaluation(records):
    ...
    rows = []
    def _final_status(spec_type, outcomes):
        if not outcomes:
            return 'Not Implemented' if spec_type == 'UC' else 'Not Enforced' if spec_type == 'BR' else 'Missing'
        passed = outcomes.count('Pass')
        partial = sum(1 for item in outcomes if item == 'Partial')
        failed = sum(1 for item in outcomes if item in ('Fail', 'Error'))
        total = len(outcomes)
        if passed == total:
            return 'Implemented Correctly' if spec_type == 'UC' else 'Enforced Correctly' if spec_type == 'BR' else 'Complete'
        if failed == total:
            return 'Incorrectly Implemented' if spec_type == 'UC' else 'Incorrectly Enforced' if spec_type == 'BR' else 'Incorrect'
        if partial > 0 or passed > 0:
            return 'Partially Implemented' if spec_type == 'UC' else 'Partially Enforced' if spec_type == 'BR' else 'Partial'
        return 'Not Implemented' if spec_type == 'UC' else 'Not Enforced' if spec_type == 'BR' else 'Missing'

    for uid, outs in sorted(uc_outcomes.items()):
        rows.append([uid, 'UC', len(outs), outs.count('Pass'), sum(1 for item in outs if item == 'Partial'), sum(1 for item in outs if item in ('Fail', 'Error')), _final_status('UC', outs), f"{outs.count('Pass')}/{len(outs)} passed"])
    for bid, outs in sorted(br_outcomes.items()):
        rows.append([bid, 'BR', len(outs), outs.count('Pass'), sum(1 for item in outs if item == 'Partial'), sum(1 for item in outs if item in ('Fail', 'Error')), _final_status('BR', outs), f"{outs.count('Pass')}/{len(outs)} passed"])
    for wid, outs in sorted(wf_outcomes.items()):
        rows.append([wid, 'WF', len(outs), outs.count('Pass'), sum(1 for item in outs if item == 'Partial'), sum(1 for item in outs if item in ('Fail', 'Error')), _final_status('WF', outs), f"{outs.count('Pass')}/{len(outs)} passed"])
    _write_csv('Artifact_Evaluation.csv', ['Artifact ID', 'Artifact Type', 'Tests', 'Pass', 'Partial', 'Fail', 'Final Status', 'Remarks'], rows)
    return rows
