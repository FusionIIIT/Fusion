...
class WorkOrderFormSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if len(alloted_time) > 200:
            raise serializers.ValidationError({'alloted_time': 'Allotted time must not exceed 200 characters'})
...
class CreateRequestsSerializer(serializers.ModelSerializer):
    class Meta:
        model = Requests
        fields = ['id', 'name', 'area', 'description', 'requestCreatedBy', 'isPriority']
