...
def finalize_proposal_and_set_routing(request_id, proposal_id, is_priority=False):
    ...
    try:
        ...
        raise ValidationError("request_id must be a valid integer")

    try:
        normalized_proposal_id = int(proposal_id)
    except (TypeError, ValueError):
        raise ValidationError("proposal_id must be a valid integer")

    request_obj = get_request_by_id(normalized_request_id)
    if not request_obj:
        raise NotFoundError(f"Request {normalized_request_id} not found")
    
    proposal = Proposal.objects.filter(id=normalized_proposal_id).first()
    if not proposal:
        raise NotFoundError(f"Proposal {normalized_proposal_id} not found")
    
    if proposal.request_id != request_obj.id:
...
def validate_iwd_admin_approval(request_id):
    ...
    if not request_obj.estimated_budget:
        return {
            "valid": False,
            "approver": "IWD Admin",
            "message": "Engineer must propose a budget before IWD Admin approval can begin"
        }
    ...
    return {
        "valid": True,
        "approver": "IWD Admin",
        "message": f"IWD Admin can approve this request (Step 1/3). Budget: Rs {request_obj.estimated_budget}"
    }
...
def validate_hod_approval(request_id):
    ...
    if not request_obj.estimated_budget:
        return {
            "valid": False,
            "approver": "HOD",
            "message": "Engineer must propose a budget before HOD/Dean approval can begin"
        }
...
def validate_director_approval(request_id):
    ...
    if not request_obj.estimated_budget:
        return {
            "valid": False,
            "approver": "Director",
            "message": "Engineer must propose a budget before Director approval can begin"
        }
