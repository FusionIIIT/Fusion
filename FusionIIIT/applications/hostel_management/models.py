...
class Hall(models.Model):
    ...
    type_of_seater = models.CharField(
        max_length=50, choices=TYPE_OF_SEATER_CHOICES, default="single"
    )
...
class GuestRoomBooking(models.Model):
    ...
    status = models.CharField(
        max_length=255,
        choices=HostelManagementConstants.BOOKING_STATUS,
        default="Pending",
    )
...
class StaffSchedule(models.Model):
    ...
    day = models.CharField(
        max_length=15, choices=HostelManagementConstants.DAYS_OF_WEEK
    )
...
class HostelNoticeBoard(models.Model):
    ...
    'scope' stores the scope of notice(local/global)
    """
...
class HostelLeave(models.Model):
    ...
    file_upload = models.FileField(
        upload_to="hostel_management/", null=True, blank=True
    )
...
class HostelAllotment(models.Model):
    ...
    def __str__(self):
        return (
            str(self.hall)
            + str(self.assignedCaretaker)
            + str(self.assignedWarden)
            + str(self.assignedBatch)
        )
...
class HostelTransactionHistory(models.Model):
    ...
    change_type = models.CharField(
        max_length=100
    )  # Example: 'Caretaker', 'Warden', 'Batch'
...
class HostelHistory(models.Model):
    ...
    caretaker = models.ForeignKey(
        Staff, on_delete=models.SET_NULL, null=True, related_name="caretaker_history"
    )
    batch = models.CharField(max_length=50, null=True)
    warden = models.ForeignKey(
        Faculty, on_delete=models.SET_NULL, null=True, related_name="warden_history"
    )
