...
FACULTY_DESIGNATIONS = {"Associate Professor", "Professor", "Assistant Professor"}


def _get_designation_name(user):
    designation = (
        HoldsDesignation.objects.select_related('designation')
        .filter(working=user)
        .first()
        or HoldsDesignation.objects.select_related('designation')
        .filter(user=user)
        .first()
    )
    return str(designation.designation) if designation else None


def _is_student_user(user_details, designation_name):
    return (
        designation_name == "student"
        or user_details.user_type == "student"
        or Student.objects.filter(id__id__iexact=user_details.id).exists()
    )


def _get_academic_student(user_details):
    return (
        Student.objects.select_related('id','id__user','id__department')
        .filter(id=user_details.id)
        .first()
        or Student.objects.select_related('id','id__user','id__department')
        .filter(id__id__iexact=user_details.id)
        .first()
    )
...
def academic_procedures(request):
    ...
    designation_name = _get_designation_name(request.user)

    if _is_student_user(user_details, designation_name):
        obj = _get_academic_student(user_details)
        if obj is None:
            return HttpResponse('student record not found')
        return HttpResponseRedirect('/academic-procedures/stu/')
        # return HttpResponseRedirect('/logout/')
    elif designation_name in FACULTY_DESIGNATIONS:
