...
def get_reference_list(request):
    if request.method == 'POST':
        student = selectors.get_student_for_user(request.user)
        reference_objects = selectors.get_reference_list_json(student)
...
def company_name_dropdown(request):
    if request.method == 'POST':
        ...
        company_name = selectors.get_company_names_by_prefix(current_value)
...
def checking_roles(request):
    if request.method == 'POST':
        current_value = request.POST.get('current_value')
        role_name = selectors.get_role_names_by_prefix(current_value)
...
def placement(request):
    ...
    current1 = selectors.get_designation_queryset(user, "placement chairman")
    current2 = selectors.get_designation_queryset(user, "placement officer")
    current = selectors.get_designation_queryset(user, "student")

    # If the user is Student
    if current:
        student = selectors.get_student_for_user(user)

        # Student view for showing accepted or declined schedule
        if request.method == 'POST':
            if 'studentapprovesubmit' in request.POST:
                services.update_invitation_status(
                    request.POST['studentapprovesubmit'], 'ACCEPTED'
                )
            if 'studentdeclinesubmit' in request.POST:
                services.update_invitation_status(
                    request.POST['studentdeclinesubmit'], 'REJECTED'
                )
        ...
        placementstatus = selectors.get_student_upcoming_placement_status(student)
    ...
    if 'schedulesubmit' in request.POST:
        ...
        if form5.is_valid():
            ...
            try:
                services.create_schedule_and_notification(
                    ...
                    location=location,
                    time=time,
                    role_name=role_offered,
                    attached_file=attached_file,
                )
            except Exception:
                messages.error(request, "Problem Occurred for Schedule Add!!!")
            else:
                messages.success(request, "Schedule Added Successfull!!")
    schedules = selectors.get_all_schedules()
...
def add_placement_schedule(request):
    ...
    current2 = selectors.get_designation_queryset(user, "placement officer")
    current = selectors.get_designation_queryset(user, "student")
    students = selectors.get_all_notify_students()
    apply_for = selectors.get_all_roles()
    context={
        'isStaff': isStaff,
        'current': current,
        'current1':current1,
...
def delete_placement_record(request):
    if 'delete_stats' in request.POST and request.POST['delete_stats']:
        try:
            ...
            selectors.delete_placement_record_by_id(record_id)
...
def add_placement_visit(request):
    ...
    user=request.user
    current1 = selectors.get_designation_queryset(user, "placement chairman")
    all_placement_visits = selectors.get_all_chairman_visits()
    context = {
...
def placement_visit_save(request):
    if request.method!="POST":
        ...
        try:
            services.create_chairman_visit(
                company_name=company_name,
                location=location,
                visiting_date=date,
                description=desc,
