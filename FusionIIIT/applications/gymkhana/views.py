...
def new_club(request):
    if request.method == 'POST':
        ...
        try:
            ...
            club_info = Club_info(club_name=club_name, category=category, co_ordinator=co_student,
                                  co_coordinator=coco_student, faculty_incharge=faculty_inc, description=d_d)
            club_info.save()
...
def event_report(request):
    if request.method == 'POST':
        ...
        # saving data to the database
        other_report = Other_report(incharge=extra, event_name=event, date=date + " " + time, event_details=report,
                                    description=d_d)
        other_report.save()
...
def club_budget(request):
    if request.method == 'POST' and request.FILES['budget_file']:
        ...
        club_budget = Club_budget(club=club_name, budget_amt=budget_amt, budget_file=budget_file, budget_for=budget_for,
                                  description=desc)
        club_budget.save()
...
def club_report(request):
    if request.method == 'POST' and request.FILES['report']:
        ...
        # saving data to the database
        club_report = Club_report(club=club_name, incharge=extra, event_name=event, date=date + " " + time,
                                  event_details=report, description=d_d)
        club_report.save()
...
def fest_budget(request):
    if request.method == 'POST' and request.FILES['file']:
        ...
        fest_budget = Fest_budget(fest=fest, budget_amt=budget_amt, budget_file=budget_file, description=desc,
                                  year=year)
        fest_budget.save()
