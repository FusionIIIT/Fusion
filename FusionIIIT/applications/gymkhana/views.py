...
def gymkhana(request):
    ...
    desig = list(HoldsDesignation.objects.all().filter(
        working=request.user).values_list('designation'))
...
def new_session(request):
    if request.method == "POST":
        ...
        try:
            ...
            message = ""
            if(res == "success"):
                session = Session_info(club=club_name, venue=venue, date=date, start_time=start_time,
                                       end_time=end_time, session_poster=session_poster, details=desc)
                session.save()
...
def date_sessions(request):
    if(request.is_ajax()):
        ...
        get_sessions = Session_info.objects.filter(
            date=value).order_by('start_time')
        dates = []
