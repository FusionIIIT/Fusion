...
def coordinator_club(request):
    ...
    for details in Club_info.objects.select_related(
        ...
        if co_ordinator[0] == str(request.user) or co_coordinator[0] == str(
            request.user
        ):
            return details
        ...
        try:
            ...
            if result == "success":
                event = Session_info.objects.select_related(
                    "club",
                    "club__co_ordinator",
                    "club__co_ordinator__id",
                    "club__co_ordinator__id__user",
                    "club__co_ordinator__id__department",
                    "club__co_coordinator",
                    "club__co_coordinator__id",
                    "club__co_coordinator__id__user",
                    "club__co_coordinator__id__department",
                    "club__faculty_incharge",
                    "club__faculty_incharge__id",
                    "club__faculty_incharge__id__user",
                    "club__faculty_incharge__id__department",
                ).get(id=session_id)
...
def delete_events(request):
    ...
    try:
        for ids in selectedIds:
            delEvent = Event_info.objects.select_related(
                "club",
                "club__co_ordinator",
                "club__co_ordinator__id",
                "club__co_ordinator__id__user",
...
def delete_memberform(request):
    ...
    try:
        for ids in selectedIds:
            delMemberform = Club_member.objects.select_related(
                "club",
                "club__co_ordinator",
                "club__co_ordinator__id",
                "club__co_ordinator__id__user",
...
def facultyData(request):
    ...
    try:
        ...
        faculty = (
            ExtraInfo.objects.select_related("user", "department")
            .all()
            .filter(user_type="faculty")
        )
        facultyNames = []
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if result == "error":
                message = (
                    message
                    + "The entered roll number of the co_ordinator does not exist"
                )
                content = {"status": result, "message": message}
            ...
            if result == "error":
                message = (
                    message
                    + "The entered roll number of the co_coordinator does not exist"
                )
                content = {"status": result, "message": message}
                content = json.dumps(content)
                return HttpResponse(content)
            ...
            message = (
                message
                + "The form has been successfully dispatched for further process"
            )
        except Exception as e:
            result = "error"
            message = "Some error occurred"
...
def return_content(request, roll, name, desig, club__, student_clubs, notifications):
    ...
    faculty = ExtraInfo.objects.select_related("user", "department").filter(
        user_type="faculty"
    )
    club_name = Club_info.objects.select_related(
        "co_ordinator",
        "co_ordinator__id",
        "co_ordinator__id__user",
        "co_ordinator__id__department",
    ...
    club_member = Club_member.objects.select_related(
        "club",
        "club__co_ordinator",
        "club__co_ordinator__id",
        "club__co_ordinator__id__user",
    ...
    club_budget = Club_budget.objects.select_related(
        "club",
        "club__co_ordinator",
        "club__co_ordinator__id",
        "club__co_ordinator__id__user",
    ...
    club_session = Session_info.objects.select_related(
        "club",
        "club__co_ordinator",
        "club__co_ordinator__id",
        "club__co_ordinator__id__user",
    ...
    club_event = Event_info.objects.select_related(
        "club",
        "club__co_ordinator",
        "club__co_ordinator__id",
        "club__co_ordinator__id__user",
    ...
    club_event_report = Club_report.objects.select_related(
        "club",
        "club__co_ordinator",
        "club__co_ordinator__id",
        "club__co_ordinator__id__user",
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if result == "success":
                ...
                gymkhana_session(
                    request.user, recipients, "new_session", club_name, desc, venue
                )
...
def club_approve(request):
    if request.method == "POST":
        ...
        for club in club_approve_list:
            ...
            co_user = User.objects.get(username=club_info.co_ordinator)
            co_co_user = User.objects.get(username=club_info.co_coordinator)

            HoldsDesignation.objects.create(
                designation_id=56, user_id=co_user.id, working_id=co_user.id
            )
            HoldsDesignation.objects.create(
                designation_id=57, user_id=co_co_user.id, working_id=co_co_user.id
            )

            Club_member.objects.create(
                club_id=club_info.club_name, member=student1, status="confirmed"
            )
            Club_member.objects.create(
                club_id=club_info.club_name, member=student2, status="confirmed"
            )
            messages.success(
                request, f"Successfully approved {club_info.club_name} club."
            )
...
def club_reject(request):
    ...
    """
    club_reject_list = request.POST.getlist("check")
...
def date_sessions(request):
    ...
    if request.is_ajax():
        ...
        get_sessions = (
            Session_info.objects.select_related(
                "club",
                "club__co_ordinator",
                "club__co_ordinator__id",
                "club__co_ordinator__id__user",
...
def date_events(request):
    ...
    if request.method == "POST":
        value = request.POST.get("date")
        get_events = (
            Event_info.objects.select_related(
                "club",
                "club__co_ordinator",
                "club__co_ordinator__id",
                "club__co_ordinator__id__user",
...
def budget_approve(request):
    ...
    """
    if request.method == "POST":
        budget_approve_list = list(request.POST.getlist("check"))
        first_words = []
        for item in budget_approve_list:
...
def filetracking(request):
    ...
    file = File.objects.select_related(
        "uploader", "uploader__user", "uploader__department", "designation"
    ).all()
