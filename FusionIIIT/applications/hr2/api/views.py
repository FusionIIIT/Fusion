...
class LeaveApplicationListCreateView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            if is_director or (is_hod and is_cl_rh_leave):
...
class AppraisalFormListCreateView(APIView):
    ...
    def get(self, request):
        ...
        if is_hr_staff:
            appraisals = AppraisalFormNew.objects.all()
        elif is_director:
            appraisals = AppraisalFormNew.objects.filter(status='REVIEWED')
        elif is_hod:
...
class AppraisalReviewView(APIView):
    ...
    def post(self, request, pk):
        ...
        appraisal.save(update_fields=['reviewer_id', 'reviewer_comments', 'rating', 'status'])
        serializer = AppraisalFormSerializer(appraisal)
        return Response(serializer.data)
