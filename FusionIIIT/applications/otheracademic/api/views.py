...
class VerifyNoDuesView(APIView):
    ...
    def post(self, request):
        try:
            ...
            # Get ExtraInfo by roll_no (which is the id/username)
            extra_info = ExtraInfo.objects.get(id=roll_no)
            no_dues = NoDues.objects.get(roll_no=extra_info)
            
            # Map department to no-dues fields
            dept_field_map = {
            ...
            if department not in dept_field_map:
                return Response(
            ...
            if department == 'acad_admin':
                first_four_statuses = _no_dues_role_statuses(no_dues)
                first_four_clear = all(
                    first_four_statuses[role_name] == 'clear'
...
class DownloadNoDuesCertificateView(APIView):
    ...
    def get(self, request):
        try:
            ...
            # Minimal valid one-page blank PDF so download never fails.
