...
def cancel_booking(request):
    if request.method == 'POST':
        ...
        if charges:
            ...
            Bill.objects.create(booking=booking, meal_bill=x, room_bill=x,
                                caretaker=user, payment_status=True)
        return HttpResponseRedirect('/visitorhostel/')
    else:
        return HttpResponseRedirect('/visitorhostel/')
...
def check_out(request):
    ...
    if user:
        if request.method == 'POST':
            ...
            Bill.objects.create(booking=booking, meal_bill=int(meal_bill),
                                room_bill=int(room_bill), caretaker=user, payment_status=True)
