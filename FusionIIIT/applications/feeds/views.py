...
@login_required
def tags_based_view(request, string):
    """
...
def remove_tag(request):
    ...
    """
    if request.method == 'POST':
        user_tags = tags.objects.select_related().all().filter(Q(user=request.user))
        tagto_delete = AllTags.objects.all().filter(Q(subtag=request.POST.get('id')))
        user_tags.filter(Q(my_subtag__in=tagto_delete)).delete()
        return JsonResponse({"done":"1"})
    else:
        return JsonResponse({"done":"0"})
        

def particular_question(request, id):
    """
...
def upvote_question(request,id):
    ...
    is_upvoted = question.likes.all().filter(username=request.user.username).count()
    if request.is_ajax() and is_upvoted == 0:
...
def downvote_question(request,id):
    ...
    is_downvoted = question.dislikes.all().filter(username=request.user.username).count()
    if request.is_ajax() and is_downvoted == 0:
...
def upvote_answer(request,id):
    ...
    is_upvoted = answer.likes.all().filter(username=request.user.username).count()
    if request.is_ajax() and is_upvoted == 0:
...
def downvote_answer(request,id):
    ...
    is_downvoted = answer.dislikes.all().filter(username=request.user.username).count()
    if request.is_ajax() and is_downvoted == 0:
...
def admin(request):
    """
    Checks the user if he or she is assigned with admin role or some other role or the user exists 
    @param:
        request- contains metadata of the requested page
    @variables- 
...
@login_required
def administrative_view(request, string):
